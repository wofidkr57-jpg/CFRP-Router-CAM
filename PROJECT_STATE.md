...
- 기본 브랜치: `main`
- 현재 애플리케이션 소스 버전: **V1.21 배포 준비**
- 배포 상태: V1.21 로컬 회귀78개·GUI6회·EXE 빌드 완료. GitHub Actions Windows 회귀·GUI6회·Defender·릴리스 대기.
...
- 기준 main: `404c50e` / 작업 시작 시 최신 릴리스 `v1.20`. 작업 브랜치 `fix/simulation-line-cache`. 공개 범위 유지, ADMIN 쓰기 권한 확인. PR 병합으로 V1.21 Windows 검증·Defender·릴리스 자동화를 실행한다.
...
- 다음 작업: V1.21 GitHub Actions Windows 검사와 Defender 결과, 릴리스 자산 및 자동 업데이트 매니페스트 해시를 확인한다. 아래 버전별 내역의 대기 항목은 과거 기록이며 현재 검증 범위는 이 절을 우선한다.
...
- 로컬 Windows 회귀78개, GUI6회, py_compile, diff 검사와 EXE 빌드 성공. GitHub Actions Windows GUI6회, Defender, 릴리스 및 자동 업데이트 매니페스트 확인 대기.
