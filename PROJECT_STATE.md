...
- 기본 브랜치: `main`
- 현재 애플리케이션 소스 버전: **V1.20 기반 + 미배포 시뮬레이션 캐시 수정**
...
- 기준 main: `404c50e` / 최신 릴리스 `v1.20`. 작업 브랜치 `fix/simulation-line-cache`. 공개 범위 유지, ADMIN 쓰기 권한 확인. 소스 PR로 제출하며 기존 릴리스/자동 업데이트 매니페스트는 유지한다.
...
- 검증: Windows Python 3.12.14, 저장소 고정 의존성으로 전체 단위 검사 **74개 통과**(추가 4개 포함). 10,000개 이동의 12회 모드 전환에서 경로/깊이 ID 유지, 급속 토글, 전진·되감기, 카메라·리사이즈 무효화 검사. `py_compile`, `git diff --check`, `build_windows.ps1` 성공.
- 검증 범위: 테스트는 화면 없는 Canvas 대역으로 소스 동작을 검사했다. 사용자 요청에 따라 GUI 스모크/EXE 실행, Mach3·실기 제어는 수행하지 않았다. 실제 UI 전환 시간 및 백신 판정은 미검증. EXE는 로컬 빌드 확인용이며 배포하지 않았다.
...
- 다음 작업: PR 검토 후 실제 GUI 전환 성능 확인, 필요 시 다음 버전으로 정식 배포. 이번 변경으로 NC 생성 로직은 바꾸지 않았다. 아래 버전별 내역의 대기 항목은 과거 기록이며 현재 검증 범위는 이 절을 우선한다.
...
- 후속 반영: `기존 누적거리/기존 누적시간` UI·설정·NC 누적 헤더를 삭제. 한 작업은 새 공구 기준 0m부터 마모를 계산하며, 2분할 PART2의 공구지름 계산만 PART1 절삭거리를 내부 전달한다. 새 공구 교체 전 여러 독립 NC 파일을 연속 가공하는 누적 보정은 지원하지 않는다.
