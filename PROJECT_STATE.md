...
- 기본 브랜치: `main`
- 현재 애플리케이션 소스 버전: **V1.29 + 미배포 작업 저장/외곽 삽입선/거리 초과 확인**
- 배포 상태: **V1.29 릴리스·자동 업데이트 반영 완료**. Windows CI 검사96개·GUI16경로·EXE 빌드·Defender 검사 통과. 실제 CNC 가공은 미검증.
...
### 후속 작업 — 2026-09-23
- `feat/job-save-order-drop`: 작업 .cfrpcam 저장/열기/다른 이름 저장/Ctrl+S/O 구현. 형상·원본 파츠·배치·설정·탭·시작점·순서·START/END 포함, 스키마/수치 검증·원자적 교체·작업 열기/종료 저장 확인. 외곽 위/아래 삽입선·끝 자동 스크롤·↑↓ 한 칸 이동·손잡이 표시 추가. 10m 이상은 기본 아니요 확인 후 생성 허용하고 NC 경고 주석 기록(사용자 요청); 기존 8m 경고/경로 검사는 유지. 회귀96개·GUI17경로·Windows EXE 빌드 통과. 별도 App에서 저장 복원/생성 경로 동일성, 포켓 자료 복원, 손상/NaN/잘못된 피드/저장 실패 시 기존 자료 보존, 18.043m 거절/승인, 삽입선 위치/순서/밖 드롭/한 칸 이동 검증. 배포는 V1.29 유지, 실가공/기계 제어 미수행.
- V1.29 배포 확인: PR #9 병합 8ed5f72, 작은 설정 탭의 안내 잘림을 0e63077에서 스크롤로 해결. 최초 CI는 게시 전에 취소하고 Actions 35825508185 회귀96개·GUI16경로·EXE 빌드·Defender 통과. 매니페스트 3b2cda4, 공개 EXE 2종 재다운로드 SHA-256 099364bf8bab8f61d1da29d88ca3bc9a7d25c9d6ffb2e0a0f94253f6ba9242d7이 매니페스트/릴리스 digest와 일치. 소스 ZIP 해시·무결성·버전·단축키/설정 스크롤·NC/개인 설정 제외 확인. 개인 OneDrive ChatGPT_Projects/CFRP_Router_CAM/v1.29 사본 해시 확인. 서버 동기화·사용자 PC 업데이트 적용·물리 키보드/실가공은 미확인.
- 사용자 정정: “나중에 한 번에” 요청은 다음 배포 시 함께 적용한다는 뜻이다. 누락한 T/S와 설정 하단 단축키 안내를 V1.29에 포함한다. Esc 점 선택 취소/Home 전체 보기 추가. 한 번 선택 종료·재선택·키 반복·입력칸/별도 창 보호를 실제 Tk 이벤트로 검증했다. 기존 가공/배치 동작은 유지하며 CI 전체 검사/빌드 후 배포한다. 겹침 위치 표시는 별도 미구현 항목이다.
- V1.28 배포 확인: PR #8 병합 b09c373, Actions 35824490671 성공, 매니페스트 438929e. 공개 EXE 2종 재다운로드 SHA-256 43c0297395a0a93beb5f77129352e68d9e44c3a5b933a9fbcc2d010c310e7bca가 latest.json/릴리스 digest와 일치. 소스 ZIP 해시·무결성·1.28 버전·외곽 순서 드래그 및 개인 설정/NC 제외 확인. 개인 OneDrive ChatGPT_Projects/CFRP_Router_CAM/v1.28 보관 사본 해시 확인. 서버 동기화·사용자 PC 업데이트 적용·실가공은 미확인.
- 사용자 요청으로 PR #8의 외곽 절단 순서 드래그를 V1.28로 배포한다. 로컬 회귀96개·GUI15경로·EXE 빌드 통과. Windows CI 검증 후 실제 실행파일 해시로 자동 업데이트를 갱신한다. 아래 미배포 표현은 배포 전 기록이다.
- `feat/outer-cut-order`: 오른쪽 목록 외곽 행의 순서/이름/Layer 영역 드래그로 절단 순서 고정, 지정 셀 숫자 편집 및 전체 자동 복원 추가. 일반 순번과 분리된 outer_cut_order로 내부 우선→지정 외곽→자동 외곽을 보장하고 최적화 재정렬 방지. 같은 번호는 자동 정렬 기준, 최종 배열 후 복사본별 지정 권장. 개체 이름/#번호·가로 스크롤·대상 강조·되돌리기·미리보기/NC 캐시/NC 주석에 연결. 회귀96개·GUI15경로 및 최종 드래그/편집 재검증·Windows EXE 빌드·구문/diff 통과. 일반/어니언스킨 분리 NC에서 내부 우선과 지정 외곽 순서 정적 확인. V1.27 배포 유지, 실제 장비 미검증. T/S 단축키/설정 안내와 겹침 위치 표시는 별도 일괄 작업 대기.
- V1.27 배포 확인: PR #7 병합 6c7057c, Actions 35820229740 성공, 매니페스트 89a13ca. 공개 EXE 2종 재다운로드 SHA-256 ac4230226d2650b74b4b42be0a68be93e2dcdd62662aa909efcdae36a1a8ca26가 latest.json/릴리스 digest와 일치. 소스 ZIP 해시·무결성·1.27 버전·스냅 제거·기본1mm/Shift0.1mm 및 개인 설정/NC 제외 확인. 개인 OneDrive ChatGPT_Projects/CFRP_Router_CAM/v1.27 보관 사본 해시 확인. 서버 동기화·사용자 PC 업데이트 적용·물리 키보드/실가공은 미확인.
- 사용자 요청으로 PR #7의 배치 스냅 제거·Num Lock 방향키 수정·기본1mm/Shift0.1mm를 V1.27로 배포한다. 검증된 CI 실행파일의 실제 해시로 업데이트를 갱신한다. 아래 미배포 표현은 배포 전 기록이다.
- `fix/remove-placement-snap`: 사용자 요청으로 배치 테두리 스냅/해제 카운터만 제거하고 공구 참고선·원점/측정 선택 스냅은 유지했다. 방향키 차단 조건이 Windows Num Lock(0x8)을 Alt로 오인하던 오류를 수정하고 기본1mm/Shift0.1mm로 변경했다. Tk Windows GetState 원문 확인, NumLock+Extended/CapsLock/Shift 이동·Alt/Ctrl 차단과 실제 Canvas 이벤트 검증. 회귀94개·GUI14경로 및 최종 방향키/다중선택 재검증·Windows EXE 빌드·구문/diff 통과. 테두리 근처 다중선택 0.3/0.2mm 자유 이동 확인. V1.26 배포 유지, 물리 키보드/실가공 미검증.
- V1.26 배포 확인: PR #6 병합 1b024a9, CI의 작은 화면에서 고정 확대 검사 실패를 fcca0ae에서 화면 맞춤 검사로 수정 후 Actions 35818646987 성공, 매니페스트 c9cef35. 공개 EXE 2종 재다운로드 SHA-256 4fff149928ce17be3c229ad3d8ffecd7c33e3113f35918c982e51e3f322510cf가 원격 latest.json/릴리스 digest와 일치. 소스 ZIP 해시·무결성·버전·NC/개인 설정 제외 확인. 개인 OneDrive ChatGPT_Projects/CFRP_Router_CAM/v1.26 사본 해시 확인. 서버 동기화·사용자 PC 설치/업데이트 적용·실가공은 미확인. 촘촘한 배열 성능과 공구/리드인 판재 이탈 차단은 별도 남은 작업이다.
- 사용자 요청으로 PR #6의 패널 폭·Ctrl 다중 선택·테두리 스냅·공구 참고선을 V1.26으로 배포한다. 로컬 회귀94개·GUI14경로·EXE 빌드 통과. Windows CI 검증 후 실제 해시로 자동 업데이트를 갱신한다. 아래 미배포 표현은 배포 전 기록이다.
- PR #6에 수동 배치 테두리 스냅과 경량 공구 중심선 미리보기를 추가했다. 판재 가장자리 여유 유지·8px/최대2mm 스냅·다중 선택 묶음 유지, 스냅 후 다음 2회 드래그 자유 조정(방향키 스냅 없음). 수동 배치에서 숨겨졌던 공구 지름/치수 보정선을 주황 점선으로 표시하며 리드인·마모·포켓·전체 충돌검사는 제외한다. 회귀94개·GUI14경로·Windows EXE 빌드·구문/diff 검사 통과. 테두리/모서리·해제2회·거리 밖·표시ON/OFF·공구 지름 변경·무거운 충돌계산 미호출 검증 완료. V1.25 배포 유지, 판재 이탈 차단/실기검증은 별도 미구현/미수행.
