...
- 검증: Windows Python 3.12.14, 저장소 고정 의존성으로 전체 단위 검사 **78개 통과**. 10,000개 이동의 캐시 회귀, 8m/10m 경계, 100m→10m 설정 변환, 분할 새 공구 초기화를 포함한다. 한국어·영어·첫 실행 GUI 스모크 **4회**, `py_compile`, `git diff --check`, `build_windows.ps1` 성공.
- 검증 범위: 최종 소스로 `dist/CFRP_Router_CAM.exe` 빌드 성공(80,247,668 bytes, SHA-256 `D6495AF09E955438615C65993834DA0350A4092F17E4D5648C9C346598C6C454`). EXE 직접 실행, Mach3·실기 제어와 절삭은 수행하지 않았다. 실제 8m/10m 소재 가공, UI 전환 시간 및 백신 판정은 미검증이며 실행본은 배포하지 않았다.
...
- 다음 작업: PR 검토 후 실제 GUI 전환 성능과 8m 경고/10m 차단 창을 확인하고, 필요 시 다음 버전으로 정식 배포한다. 아래 버전별 내역의 대기 항목은 과거 기록이며 현재 검증 범위는 이 절을 우선한다.
