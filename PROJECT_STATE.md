...
- 기본 브랜치: `main`
- 현재 애플리케이션 소스 버전: **V1.30 + 미배포 다중 선택 가공 버튼·균등 거리 중간 공구 교체**
- 배포 상태: **V1.30 릴리스·자동 업데이트 반영 완료**. Windows CI 검사96개·GUI17경로·EXE 빌드·Defender 검사 통과. 실제 CNC 가공은 미검증.
...
### 후속 작업 — 2026-09-23
- 중간 공구 교체: 사용자 승인 M881/기계 Z-70 복귀를 설정 탭 별도 항목으로 추가. 기본 꺼짐·최대6.5m·안정3초, 설정/작업 저장 및 기존 작업의 꺼짐 이행. 윤곽 순서를 유지하고 최대 거리 내 균등 분할, 공구별 마모 초기화/거리 경고, 마지막 불필요 교체 없음. 단일 윤곽 초과는 거절. 일반 가공과 어니언스킨 분리 지원; 단일 파일 외곽 후정삭 병용은 명시적으로 거절한다.
- 교체 NC: 안전 Z → M5/M9 → 단일 M 매크로 → mm/G90/G54 모드 복원 → 기계 Z-70 → 절삭유/스핀들·대기 → 다음 XY → 접근 Z/절입. M881은 Bottom 기준이며 P99 등 인자는 거절. 실제 교체 대기/재프로빙/실패 NC 중단은 설치된 매크로 책임. 교체/프로빙은 CAM 시뮬레이션·예상 시간에 포함되지 않음. 실제 기계 조작/실가공 검증 없음.
- 검증: 회귀104개, 관련 GUI8경로(새 설정/생성/재시작/구형 작업, 작업 파일, 다중 선택, 어니언스킨, 기본 화면, 첫 실행 ko/en/close), Windows EXE 빌드 성공. 마지막 계획 재사용 수정 후 회귀104개·생성/어니언 GUI2경로·빌드 재확인. 개인 복구 작업144윤곽은 최대6.5m에서 6.282121/5.882681/5.882681m, 133/139번째 윤곽 앞 교체2회, 총18.047484m. 마모 초기화와 복귀 XY 전 Z 하강 없음 정적 검증. 개인 NC/검증 결과는 Git 제외 local-handoff/toolchange-validation/, 로그는 local-handoff/toolchange-*.log. PR #11에 묶으며 V1.30 배포 유지, 다음 배포에 포함할 소스 변경이다.
- `feat/batch-machining-selection`: 화면/목록 다중 선택에 가공 적용/미적용 일괄 버튼 추가. 셀 선택창 생성 당시 대상 유지, 선택/미선택 보존·Undo·NC 캐시 무효화·작업 저장 연결. 실제 Tk 클릭/버튼과 NC 제외·혼합 상태·빈 선택·한영 버튼 판별 검증. 회귀96개 및 관련 GUI6경로·Windows EXE 빌드 검증. V1.30 배포 유지, 실제 기계 조작 없음.
- V1.30 배포 확인: PR #10 병합 9c74799, Actions 35836288642 회귀96개·GUI17경로·빌드·Defender 성공. 릴리스 요약 API assets가 빈 배열을 반환해 재빌드를 시도했으나 실제 assets API에는 최초 정상 첨부가 존재했음(재업로드 422). 246d0fb에서 실제 첨부 API 검증과 게시 후 매니페스트 갱신으로 보완하고 최초 검증 파일 해시로 복원. 공개 EXE 2종 직접 다운로드 SHA-256 77abc43159e98dbe7bb196556e980ef008420b3ed17e166193714cdf98cc3cd9 일치. 소스 ZIP 무결성/1.30/작업 저장·열기/삽입선/개인 복구 파일 제외 확인. 개인 OneDrive ChatGPT_Projects/CFRP_Router_CAM/v1.30 보관 사본 해시 일치. 서버 동기화·사용자 PC 업데이트 적용·실가공은 미확인.
- 사용자 요청으로 PR #10 작업 저장/열기·외곽 삽입선·10m 초과 확인 생성을 V1.30으로 배포한다. 로컬 회귀96개·GUI17경로·EXE 빌드 통과. 실제 복구 작업 파일도 열기/저장/전체 값 일치를 확인했으며 개인 복구 데이터는 공개 저장소/배포 ZIP에 포함하지 않는다. Windows CI 검증 후 실제 해시로 자동 업데이트를 갱신한다.
