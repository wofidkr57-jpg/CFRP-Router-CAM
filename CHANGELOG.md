## 미배포 — 균등 거리 공구 교체·다중 선택 가공 적용/미적용

- 설정 탭에 중간 교체 활성화/최대 거리/M 매크로/기계 복귀 Z/스핀들 대기 추가. 기본 꺼짐, 6.5m/M881/-70/3초. 기존 작업 파일은 꺼짐으로 이행한다.
- 윤곽 순서를 유지하며 최대 거리 안에서 구간을 균등화하고 마모 보정을 새 공구 기준으로 다시 계산한다. 18m/최대8m는 6m씩 3구간, 교체 2회. 단일 윤곽 초과/잘못된 매크로·좌표 및 단일 파일 외곽 후정삭 병용은 거절한다.
- 안전 Z·M5/M9·매크로·기계 Z 복귀 후 절삭유/스핀들을 복원하고 다음 XY를 이동한 뒤 Z 절입한다. 계산한 마모 계획을 거리 표시와 경로 생성에 재사용한다. 공구별 거리 경고와 생성 전 구간 안내 추가.

- 목록 위 일괄 적용/미적용 버튼 추가. 화면/목록 다중 선택 유지, 미선택 보존, 되돌리기와 NC 캐시 무효화 연결.
- 셀 선택창을 열 때 대상 목록을 고정하고 한영 표시와 무관하게 적용/제외를 판정한다.

## V1.30 — 2026-09-23 · 작업 저장·드래그 삽입선·거리 초과 확인

- 데이터 전용 `.cfrpcam` 작업 저장/열기/다른 이름 저장과 Ctrl+S/O 추가. 형상/배치/원본 파츠/설정/탭/시작점/순서/START·END 복원, 스키마 검증, 원자적 파일 교체 및 변경 작업 확인.
- 외곽 행 위/아래 절반의 노란 삽입선과 자동 스크롤, 한 칸 이동 버튼, 드래그 손잡이 표시 추가.
- 10m 거리 제한을 기본 ‘아니요’인 사용자 확인으로 전환. 초과 승인 코드에 경고 주석 기록.

## V1.29 — 2026-09-23 · 단축키와 설정 안내

- T 수동탭·S 시작점의 1회 선택 단축키, Esc 점 선택 취소, Home 전체 보기를 추가했다.
- 입력칸/별도 창/드래그/Ctrl·Alt 조합 보호, 키 반복 시 선택 모드 유지와 설정 탭 하단 한영 단축키 안내를 추가했다.

## V1.28 — 2026-09-23 · 외곽 절단 순서 드래그

- 목록 외곽 행을 위/아래로 드래그하여 절단 순서를 고정하고 전체 자동 복원 버튼을 추가했다. 내부 우선은 유지한다.

- 외곽 `지정` 셀에서 절단 순번을 입력한다. 내부 우선 후 지정 외곽, 나머지 자동 외곽 순서로 처리하며 최적화로 지정 순서가 바뀌지 않는다.
- 개체 이름/복사본 번호와 가로 스크롤 추가, 미리보기/코드 캐시/NC 주석/어니언스킨 분리/되돌리기에 연결했다.

## V1.27 — 2026-09-23 · 배치 스냅 제거·방향키 수정

- 테두리 스냅과 2회 해제 처리를 제거했다. 드래그 이동·다중 선택·방향키·공구 참고선은 유지한다.
- Windows Num Lock을 Alt로 잘못 해석해 방향키를 무시하던 조건을 수정했다. 기본 1mm, Shift 0.1mm로 이동량을 변경했다.

## V1.26 — 2026-09-23 · 패널 폭·다중 선택·스냅·공구 참고선
